...
# --- HELPERS (Duplicated to allow standalone running) ---
@st.cache_resource
...
# --- VIEWER UI ---
def main():
    # 1. Setup Page - Force Collapsed Sidebar
    st.set_page_config(page_title="Quote Viewer", page_icon="🏥", layout="wide", initial_sidebar_state="collapsed")

    # 2. Get Quote ID
    if "quote_id" not in st.query_params:
        st.error("❌ No Quote ID provided. Please use the link generated by the tool.")
        return
    ...
    # 3. Hide Streamlit UI Elements (Sidebar, Header, etc.)
    st.markdown("""
        <style>
    ...
    random_quote = random.choice(quotes_list) if quotes_list else "Health is wealth."
